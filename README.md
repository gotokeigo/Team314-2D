...
2. SorceTreeで `develop`から `feature/作業内容` ブランチを作成する。(絶対)
  >前回使用したのと同じブランチを使う場合は以下の手順で行ってください
  >
  > developをプル（最新の状態にする）  
...
>
4. 作成したブランチにdevelopをプルする。
5. Discordの作業報告チャンネルに作業するシーンやPrefabを投稿する。
6. 作業開始
7. 作業したファイルをコミット(ローカルコミット、リモートコミットは作成したブランチを選択、プルダウンの選択肢にない場合は入力してください)、gitでプルリクを送る(develop)  
   Gitでプルリクを送る時プッシュする先がdevelopになっているかを確認してください  
   　プルリクを送る時にコンフリクトが起きた場合はSourceTreeのターミナルで  
   　`git mergetool`  
   　を実行し解決した後、再度コミットしプルリクを送ってください  
8. マージされたらDiscordに報告する  
